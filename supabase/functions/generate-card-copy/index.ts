...
const GROUNDING_RULE = " Write at a 5th-grade reading level: short, everyday words and short sentences, nothing a shopper would have to look up (say 'soft' not 'plush', 'lasts a long time' not 'durable', 'made just for you after you order' not 'made-to-order production'). Every line must be TRUE for this specific product according to the supplied details AND must help a shopper decide. Sell the design and who it is for, not the plain blank item it is printed on: what the design says or shows, who would love it, and when they would wear or use it. Supplier details about the blank (fabric blend, weight, print method, brand or model of the blank) are background only and never the headline. Use only facts the details actually support. Do NOT mention gift boxes, packaging, personalization, custom names, free shipping, guarantees, or shipping speed unless the details explicitly say so. Write for a shopper, not a designer or a printer: never use production or print-process jargon (DTG, direct-to-garment, DTF, sublimation, screen printing, CMYK, DPI, GSM, ring-spun, thread count, blank or brand model numbers, print-on-demand). Say what the shopper will see and feel instead, for example 'crisp, vivid print', and only when the details support it; if a detail is only about how it is made, leave it out. Sound like a friendly person talking to a shopper: warm, simple, specific, with the benefit first and about 6 to 10 words per line ('A bold design people will ask about' beats 'eye-catching graphic'; 'Easy to wear every day' beats 'versatile casual apparel'). Those examples show the tone only. Never copy their wording unless it is true for this product. Leave out empty filler and anything every shopper already assumes ('made with care', 'premium quality', 'built to last', 'strong stitching', 'well made', 'good quality'); it does not help anyone choose. Say what makes THIS one different or where they would use it, and only mention a baseline quality if it is a real, unusual difference. If the details support fewer points than requested, return fewer and leave the unused fields as empty strings — never pad or invent.";

// The features card: a heading written for THIS product plus bullets aimed at real life.
// The old version used a fixed "Made With Care" heading and was told to talk about speed and
// satisfaction, which produced lines like "Shipped fast" and "Guaranteed satisfaction" on
// every card. Those have their own cards, and a guarantee shouldn't be invented anyway.
const FEATURES_RULE = "a short heading (2 to 5 plain, warm words about how this item is used or how it makes the buyer feel, for example 'Made for Movie Night' or 'Wear Your American Pride'; these are examples only, do not copy them, and never just use 'Made With Care') and 5 short bullet points, each under 60 characters. Aim the bullets at real life: at least two should put the buyer in a moment where they would actually use it (for example a Friday night movie on the couch, a cold morning, a cookout, opening it as a gift), and the rest should be the design or theme, a comfort or feel detail the product details support, or something real the buyer can choose or add, like a name. Do not put shipping speed, guarantees, or promises in this card; those have their own cards. The plain blank item is background, not the product.";

const CARD_PROMPTS: Record<string, string> = {
  features: `You are a copywriter for ecommerce product listings (Etsy or Shopify). Given a product name and details, write the copy for a "features" listing card: ${FEATURES_RULE}${GROUNDING_RULE} Return ONLY valid JSON: {"heading":"...","b1":"...","b2":"...","b3":"...","b4":"...","b5":"..."}`,
