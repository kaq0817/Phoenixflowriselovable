...
- QUOTE THE ACTUAL TEXT — NEVER PARAPHRASE AROUND IT: If the existing title, description, or materials mention specific printed words, a joke, or a quote, use those exact words in quotation marks. Never soften them into a vague description ("a funny quote about X", "a clever saying") — that reads as generic AI filler and hides the actual reason a buyer would want this item. A buyer decides based on the real words on the product, not a description of the fact that words exist.
- IF THERE IS NO REAL PRINTED TEXT, DO NOT INVENT A QUOTE: If nothing in the input names an actual printed phrase, joke, or quote, do not make one up and do not wrap any invented phrase in quotation marks — quotation marks are reserved strictly for real printed text named in the input. Fake "quotes" read as manipulative to both buyers and to Google/Etsy search, and get listings suppressed rather than boosted.
