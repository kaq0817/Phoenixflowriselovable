...
DO NOT regress into a supplier-spec recap. Raw product details are background context only. If the source listing is mostly blank-stock facts, manufacturing notes, or generic spec-sheet language, you must ignore that as the primary sales message and write customer-facing copy around the design, theme, use case, and the exact reason this item wins the search. The audience is not reading a spec sheet; they are deciding whether this is the product that belongs in their life.

NO PASSIVE DESCRIPTION MODE. Do not write a neutral catalog paragraph that merely describes the item. The output must behave like a sharp salesperson who has already identified the buyer's objection, answered it in the copy, and made the purchase feel like the obvious next step. If the text sounds like a bland supplier recap, rewrite it until it sounds like a person selling this item directly to a shopper who is already leaning yes.

KEYWORD TARGETING (do this before anything else):
Identify 3-5 keywords for this product that a buyer types when they are ready to purchase — not researching, not browsing, BUYING. Target keywords with estimated US monthly search volume between 500 and 5,000. This is the range where a newer store with low domain authority can actually rank — high-volume terms (25,000+) are locked up by Amazon, Wayfair, and established Etsy sellers. Avoid keywords under 200/month (no traffic) and over 10,000/month (too competitive to crack without backlinks). Favor 4-6 word hyper-specific phrases where the big players are not competing: "personalized family name metal wall sign", "custom name fleece blanket dad birthday gift" — not "wall art" or "blanket". The more specific the phrase, the lower the competition and the more buyer-ready the intent. Build every field below around these keywords.

SHOPIFY SEO RULES:
- TONE AND AUDIENCE COME FROM THE PRODUCT, NEVER FROM THE STORE: Decide who this is for and how it should sound from the product itself — its images, design, theme, and use. A kids' item speaks to the parent buying it, a cozy home item sounds warm and homey, a gaming-themed item can sound like it's for gamers. Never assume the store has a fixed niche, never assume the buyer is a gamer (or any other group) unless the product itself says so, and never carry a tone over from the store name or from other products. One store can sell very different things; each product is written for its own buyer. Only use gaming language when the product is actually gaming-themed.
- PRODUCT TYPE (product_type field): This field categorizes the product on Shopify and feeds Google Shopping — getting it wrong is a critical, visible error (e.g. labeling a comforter as an "ornament"). Determine it the same way as everything else: the product images are the primary source of truth, then the real title/description content, in that order — never guess a category unrelated to what the images and text actually show. Use a specific, accurate, common category term (e.g. "Comforter", "Ceramic Mug", "Fleece Blanket", "Metal Wall Sign") — not a vague genre word and not something borrowed from a different product entirely. If the existing product_type supplied above is already specific and accurate, keep it rather than inventing a new one. If you are not confident what category fits from the available images/text, keep the existing supplied product_type rather than replacing it with a guess — an unchanged-but-correct value is always better than a confident wrong one.
- TITLE: Descriptor-first product name only. Under 60 chars (GMC hard limit). No vendor/brand names. Format: [Descriptor] [Item Type] [Key Attribute if critical — e.g. color+size for apparel, Waterproof/Insulated for drinkware/outerwear]. Strip "Iron Phoenix GHG", "Iron Phoenix", "ghg", "| Iron Phoenix", or any store name. Example: "Wildflower Meadow Travel Mug" or "Aurora Flow Gradient Athletic Shorts Black XS-4XL". BUNDLE TITLE RULE: If the 🚨 BUNDLE warning is present above, the title MUST include ALL item types separated by ", " and "&". Example: "Meditation Lotus Leggings, Crop Top & Hoodie Blue Black". Collapsing a bundle to one item type is a critical error.
...
- DESIGN NAMES — IMAGE IS THE ONLY SOURCE OF TRUTH: The product title and existing description may contain completely wrong design names (suppliers stuff popular keywords like "Sunflower", "Butterfly", "Inspirational Quotes" into titles to game search — these are often lies). You MUST visually identify the actual design from the product images and use THAT. If the image shows a lighthouse, title it as coastal wall art. If it shows a Christmas tree, title it as Christmas decor. NEVER use a design name from the title or description unless the image confirms it. If you cannot clearly identify the design from the images, write generic specs-only copy with no design name at all.
...
- PRIMARY KEYWORD STRATEGY (do this first, before writing anything else): Identify ONE primary keyword for this product — the exact phrase a real shopper would type into Google to find it. Choose a specific long-tail phrase (3-5 words) that balances search intent with realistic competition. Example: "personalized family name metal wall sign" beats "wall art" (too broad, impossible to rank) and "vintage farmhouse iron scroll wall decor" (too niche, no volume). Once chosen, this keyword MUST appear: (1) at the very START of the seo_title, (2) in the first sentence of the seo_description, (3) in the <h3> or opening <p> of body_html, (4) in the url_handle. Supporting secondary keywords (2-3 related phrases) should appear naturally in the body bullets and closing paragraph. This is the single most important thing you do — a page optimized around one specific keyword outranks a page that mentions many keywords weakly.
- SEO TITLE (seo_title): Max 60 chars. START with the primary keyword — Google weights the beginning of the title tag most heavily. Use | as the only separator. ${storeName ? `Append "| ${storeName}" only if it fits within 60 chars.` : "Do not append any store name suffix."} Never use "Iron Phoenix GHG" anywhere. Example: "Personalized Family Name Metal Wall Sign | Our Phoenix Rise" not "Our Phoenix Rise | Personalized Family Name Wall Art".
...
RULES: (1) LENGTH IS NOT OPTIONAL: the full body_html (paragraphs B, C, D, E combined, not counting the h3) must be 220-300 words / roughly 1400-1900 characters. This is a floor, not a suggestion — before finalizing, count the words. If you're under 220, you are missing required substance (re-read the per-section word counts above and add the missing detail to whichever section is thinnest) — never pad with filler adjectives or repeated phrases to hit the count, add one more real, specific detail instead. (2) No hollow hype that isn't tied to something specific about this product. (3) No section labels or headers inside the copy — no "Key Features:", no "Benefits:". (4) COLOR AND MATERIAL MUST BE NAMEABLE IN PLAIN WORDS, NOT DROPPED AND NOT FACTORY SPEC: Carry every real fact (color, material, dimensions, care, fit) from the existing description forward SOMEWHERE in the output — but never as a raw bullet per the bullet rule above, and never as a raw factory/supplier spec (e.g. "65/35 cotton-poly ring-spun jersey", "OEKO-TEX certified dye"). Say it the way a regular customer would actually understand it — a soft cotton-blend fabric, a warm navy blue — worked naturally into a sentence in paragraph B, D, or E. The actual color name and the actual material must still be readable in plain words somewhere in the text; a benefit phrase alone ("soft enough to sleep in") is not enough if it drops the literal color or material a shopper (and Google Shopping) needs to see. No fact should be dropped, and no fact should turn into a spec-sheet phrase either — plain and specific, both at once. (5) No <html>, <body>, <head> wrappers. No inline styles. No <img> tags. No markdown, no asterisks.
...
- URL HANDLE: Hyphenated, lowercase, PRIMARY KEYWORD as the base, max 50 chars. Shorter is better — 3-5 words ideal. Example: "personalized-family-name-metal-wall-sign" not "personalized-family-name-iron-wall-sign-rustic-farmhouse-metal-wall-decor-art".
