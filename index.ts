...
serve(async (req) => {
  ...
  try {
    ...
    const { listingId, originalData, optimizedData, connectionId }: { listingId: string, originalData: Record<string, unknown>, optimizedData: Record<string, unknown>, connectionId?: string } = await req.json();
    ...
    // Extract shop ID from scopes (format: "shops_r:SHOPID ...") for consistency
    const shopMatch = connection.scopes?.match(/shops_r:(\d+)/);
    const shopId = shopMatch?.[1]; // shopMatch[1] would be the captured numeric ID
    if (!shopId) throw new Error(`Etsy shop ID not found in connection scopes. Scopes: ${connection.scopes || 'null'}`);
