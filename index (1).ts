...
// Modified to accept a geo parameter for regional targeting
async function verifyKeywordsWithSerpApi(
  ...
  for (const keyword of keywords.slice(0, 5)) { // limit to 5 to preserve API credits
    try {
      ...
      try {
        const tiktokRes = await fetch(`https://serpapi.com/search.json?engine=google&q=${encodeURIComponent(keyword + " site:tiktok.com")}&api_key=${serpApiKey}&num=3`
        );
        if (tiktokRes.ok) {
...
async function analyzeDescriptionWithGemini(description: string, geminiApiKey: string): Promise<string | null> {
  if (!geminiApiKey || !description || description.length < 50) return null; // Only analyze longer descriptions

  try {
    const genAI = new GoogleGenerativeAI(geminiApiKey);
    const model = genAI.getGenerativeModel({ model: "gemini-pro" });

    const prompt = `Analyze the following Etsy listing description for clarity, engagement, and potential improvements. Provide a concise summary of any issues or suggestions. If the description is good, respond with "Description looks good."
    ...
    console.error("Gemini API error during description analysis:", e);
...
serve(async (req) => {
  ...
  try {
    ...
    const GEMINI_API_KEY = Deno.env.get("GEMINI_API_KEY"); // Retrieve Gemini key here
    ...
    for (const listing of listings) {
      ...
      // 6. AI-powered description analysis (Gemini)
      if (GEMINI_API_KEY && description.length > 50) { // Only analyze longer descriptions
        try {
          const aiAnalysis = await analyzeDescriptionWithGemini(description, GEMINI_API_KEY);
          if (aiAnalysis && aiAnalysis !== "Description looks good.") {
            listingFindings.push({
              type: "ai_description_analysis",
              severity: "info",
              field: "description",
              message: `AI Description Analysis: ${aiAnalysis}`,
            });
          } else if (aiAnalysis === "Description looks good.") {
             listingFindings.push({
              type: "ai_description_analysis",
              severity: "info",
              field: "description",
              message: "AI Description Analysis: Description looks good.",
            });
          }
        } catch (e) {
          console.error("AI description analysis failed:", e);
        }
      }

      // 7. AI-powered title analysis (Gemini)
      if (GEMINI_API_KEY && title.length > 10) {
        try {
          const aiAnalysis = await analyzeTitleWithGemini(title, description, GEMINI_API_KEY);
          if (aiAnalysis && aiAnalysis !== "Title looks great.") {
            listingFindings.push({
              type: "ai_title_analysis",
              severity: "warning",
              field: "title",
              message: `AI Title Analysis: ${aiAnalysis}`,
            });
          } else if (aiAnalysis === "Title looks great.") {
            listingFindings.push({
              type: "ai_title_analysis",
              severity: "info",
              field: "title",
              message: "AI Title Analysis: Title looks great.",
            });
          }
        } catch (e) {
          console.error("AI title analysis failed:", e);
        }
      }

      // 8. AI-powered tag suggestions (Gemini)
      if (GEMINI_API_KEY && tags.length < 13 && (title || description)) {
        try {
          const suggestedTags = await suggestTagsWithGemini(title, description, tags, GEMINI_API_KEY);
          if (suggestedTags && suggestedTags.length > 0) {
            listingFindings.push({
              type: "ai_tag_suggestions",
              severity: "info",
              field: "tags",
              message: `AI Tag Suggestions: Consider adding these tags: ${suggestedTags.join(", ")}`,
              data: suggestedTags,
            });
          }
        } catch (e) {
      ...
      // 9. AI-powered image alt text suggestions (Gemini Vision)
      const primaryImage = listing.images?.[0]?.url_fullxfull || listing.images?.[0]?.url_170x135;
      if (GEMINI_API_KEY && primaryImage) {
        try {
          const altTextSuggestion = await analyzeImageAltTextWithGemini(primaryImage, title, description, GEMINI_API_KEY);
          if (altTextSuggestion && altTextSuggestion !== "AI image alt text analysis failed due to an API error.") {
            listingFindings.push({ type: "ai_alt_text_suggestion", severity: "info", field: "images", message: `AI Alt Text Suggestion: "${altTextSuggestion}"` });
          }
        } catch (e) {
          console.error("AI alt text suggestion failed:", e);
        }
      }
    ...
    if (userEmail) {
      try {
        // This function will call the `send-scan-email` Deno function (index (2).ts)
        // which now uses Resend, making it standalone from Lovable.
        const SUPABASE_URL = Deno.env.get("SUPABASE_URL");
        if (SUPABASE_URL) {
          await fetch(`${SUPABASE_URL}/functions/v1/send-scan-email`, {
            method: "POST",
            headers: {
